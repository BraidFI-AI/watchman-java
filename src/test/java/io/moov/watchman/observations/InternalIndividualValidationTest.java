...
/**
 * DriftGuard — Internal Individual Validation
 *
 * Day Watcher's self-policing model validation suite for individual names.
 * Catches scoring regressions and false positives before they reach production.
 *
 * Test cases defined in: observations/internal-individuals.csv
 * Delegates to DriftGuardService — same logic used by the admin UI.
 */
@SpringBootTest(classes = WatchmanApplication.class)
@DisplayName("DriftGuard — Individual Validation")
class InternalIndividualValidationTest {

    @Autowired
    private DriftGuardService driftGuardService;

    @Test
    @DisplayName("DriftGuard Individuals — All Rows")
    void validateAllIndividualRows() throws IOException {
        List<DriftGuardResult> results = driftGuardService.runSuite("individuals", "observations/internal-individuals.csv");

        System.out.println("\n" + "=".repeat(100));
        System.out.println("DRIFTGUARD — INDIVIDUAL VALIDATION");
        System.out.println("Source: observations/internal-individuals.csv");
