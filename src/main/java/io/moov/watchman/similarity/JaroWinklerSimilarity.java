...
public class JaroWinklerSimilarity implements SimilarityService {
    ...
    public double jaroWinkler(String s1, String s2) {
        ...
        if (norm1.equals(norm2)) {
            // Apply exact match favoritism if configured (default 0.0 = no boost)
            return Math.min(1.0, 1.0 + config.getExactMatchFavoritism());
        }
    ...
    public double tokenizedSimilarity(String s1, String s2, ScoringContext ctx) {
        // Add config metadata to trace if enabled
        if (ctx.isEnabled()) {
            ctx.withMetadata("similarity.boost-threshold", config.getJaroWinklerBoostThreshold())
               .withMetadata("similarity.prefix-size", config.getJaroWinklerPrefixSize())
               .withMetadata("similarity.length-penalty", config.getLengthDifferencePenaltyWeight())
               .withMetadata("similarity.phonetic-disabled", config.isPhoneticFilteringDisabled());
        }
    ...
    private double customJaroWinkler(String s1, String s2) {
        ...
        // Apply length difference penalty if ratio < cutoff (default 0.9)
     ...
     * Only applies boost if base Jaro score >= configured threshold (default 0.7).
     */
    private double applyWinklerBoost(double jaroScore, String s1, String s2) {
        // Only apply boost if Jaro score is above threshold (default 0.7)
        if (jaroScore < config.getJaroWinklerBoostThreshold()) {
            return jaroScore;
        }

        // Find common prefix length (configurable, default 4 characters)
        int prefixLen = 0;
    ...
    private boolean isStopword(String token) {
        // If configured to keep stopwords, never treat any word as a stopword
        if (config.isKeepStopwords()) {
            return false;
        }
