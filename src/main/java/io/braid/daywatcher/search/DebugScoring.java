...
public class DebugScoring {
     ...
     */
    public static double debugSimilarity(Writer w, Entity query, Entity index, io.braid.daywatcher.config.WeightConfig weightConfig) {
        ...
        EntityScorer scorer = new EntityScorerImpl(
            new JaroWinklerSimilarity(
                ...
                new io.braid.daywatcher.config.SimilarityConfig()
            ),
            weightConfig
        );
        ScoreBreakdown breakdown = scorer.scoreWithBreakdown(query, index, io.braid.daywatcher.trace.ScoringContext.disabled());
