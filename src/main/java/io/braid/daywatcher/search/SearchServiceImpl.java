...
import io.braid.daywatcher.performance.PerformanceTimer;
import io.braid.daywatcher.performance.PerformanceTimers;
...
public class SearchServiceImpl implements SearchService {
    ...
    private final io.braid.daywatcher.config.AutoClearanceConfig autoClearanceConfig;
    private final io.braid.daywatcher.config.SearchConfig searchConfig;
    private final io.braid.daywatcher.scorer.AddressComparer addressComparer;
    ...
    private final io.braid.daywatcher.similarity.JaroWinklerSimilarity jaroWinkler;

    public SearchServiceImpl(EntityIndex entityIndex, EntityScorer entityScorer,
                             io.braid.daywatcher.config.AutoClearanceConfig autoClearanceConfig,
                             io.braid.daywatcher.config.SearchConfig searchConfig,
    ...
    public List<SearchResult> search(String query, SourceList sourceList, EntityType entityType, 
        ...
        io.braid.daywatcher.similarity.TextNormalizer queryNormalizer = new io.braid.daywatcher.similarity.TextNormalizer();
    ...
    private AutoClearanceStatus applyAutoClearance(io.braid.daywatcher.model.Entity entity,
                                                    String queryAddress,
    ...
    private AutoClearanceStatus applyAddressClearance(io.braid.daywatcher.model.Entity entity,
                                                      String queryAddress) {
        ...
        io.braid.daywatcher.model.Address queryAddr;
        if (parts.length >= 2) {
            ...
            queryAddr = new io.braid.daywatcher.model.Address(line1, null, city, state, zip, "US");
        } else {
            // Simple fallback: treat whole string as line1
            queryAddr = io.braid.daywatcher.model.Address.of(queryAddress, "", "US");
        }
        
        // Normalize addresses
        io.braid.daywatcher.model.PreparedAddress preparedQueryAddr = 
            io.braid.daywatcher.scorer.AddressNormalizer.normalizeAddress(queryAddr);
        
        java.util.List<io.braid.daywatcher.model.PreparedAddress> preparedEntityAddrs = entity.addresses().stream()
            .map(addr -> io.braid.daywatcher.scorer.AddressNormalizer.normalizeAddress(addr))
            .toList();
    ...
    private AutoClearanceStatus applyDobClearance(io.braid.daywatcher.model.Entity entity,
                                                   java.time.LocalDate queryDob) {
    ...
    private AutoClearanceStatus applyGovIdClearance(io.braid.daywatcher.model.Entity entity,
                                                     String queryGovId) {
        // Collect government IDs from entity (check multiple sources)
        java.util.List<io.braid.daywatcher.model.GovernmentId> entityGovIds = new java.util.ArrayList<>();
