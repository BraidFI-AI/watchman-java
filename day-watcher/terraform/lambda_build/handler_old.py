    ...
    try:
        ...
        for page in braid_client.fetch_individuals(status='ACTIVE', updated_after=updated_after):
            ...
            if page_num <= 3:  # Sample first 3 pages
                sample_ids = [str(page[i]['id']) for i in range(min(3, len(page)))]
                print(f"     Sample IDs: {', '.join(sample_ids)}", flush=True)
            for individual in page:
                ...
                if len(batch) >= 25:
                    ...
                    print(f"    ✍️  Batch write: {written}/25 individuals persisted (total: {write_counts['individual']})", flush=True)
                    batch = []
