...
## Summary
Simple web-based Admin UI for managing ScoreConfig (26 parameters: 10 similarity + 13 weights + 3 auto-clearance), viewing ScoreTrace sessions, and testing search functionality. Built with REST API-first approach using vanilla HTML/JavaScript consuming Spring Boot endpoints.

## Scope
- REST API endpoints for config management (GET, PUT, POST)
- SPA frontend at /admin.html (no framework dependencies)
- View and edit SimilarityConfig (10 params) + WeightConfig (13 params) + AutoClearanceConfig (3 params)
...
- GET /api/admin/config - Returns all 26 config parameters (similarity + weights + auto-clearance)
- PUT /api/admin/config/similarity - Update 10 similarity parameters
- PUT /api/admin/config/weights - Update 13 weight parameters
- PUT /api/admin/config/auto-clearance - Update 3 auto-clearance parameters (phase1Threshold, addressMismatchThreshold, dobDifferenceThresholdYears)
- POST /api/admin/config/reset - Reset all to application.yml defaults
- Validation: threshold ranges (0-1), no negative weights, prefix size >= 0, dobDifferenceThresholdYears >= 0
- Error handling: IllegalArgumentException → 400 Bad Request via GlobalExceptionHandler

**DTOs**:
- AdminConfigResponse - combines SimilarityConfigDTO + WeightConfigDTO + AutoClearanceConfigDTO
- AutoClearanceConfigDTO - 3 parameters for Auto Clearance feature thresholds
...
- ScoreConfig tab contains three sections: Similarity Config, Weight Config, Auto-Clearance Thresholds
...
**State management**:
- Changes apply immediately to running SimilarityConfig/WeightConfig/AutoClearanceConfig beans
...
**TDD Approach**:
- RED: 9 failing tests in AdminConfigControllerTest (7 ScoreConfig + 2 Auto-Clearance)
- GREEN: Minimal controller + DTO implementation
- All tests pass (verified via `mvn test -Dtest=AdminConfigControllerTest`)
- Auto-Clearance tests: `shouldUpdateAutoClearanceConfig`, `shouldRejectInvalidAutoClearanceConfig`
...
curl -X PUT http://localhost:8084/api/admin/config/auto-clearance \
  -H "Content-Type: application/json" \
  -d '{"phase1Threshold":0.90,"addressMismatchThreshold":0.40,"dobDifferenceThresholdYears":2}'
...
curl http://localhost:8084/api/admin/config | jq '.autoClearance'
```
...
5. Scroll to Auto-Clearance section
6. Change phase1Threshold from 0.85 to 0.90
7. Click "Save Auto-Clearance" - expect success message
8. Click "Reload" - verify phase1Threshold shows 0.90
9. Go to Test Search tab
10. Enter "Nicolas Maduro", check "Enable ScoreTrace"
11. Click "Run Search" - should show results + trace link
12. Click trace link - opens HTML report in new tab
13. Return to Configuration tab
14. Click "Reset to Defaults" - confirm, reload - verify nameWeight back to 35 and phase1Threshold back to 0.85
