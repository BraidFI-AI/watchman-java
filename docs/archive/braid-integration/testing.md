...
**Submit bulk job:**
```bash
curl -X POST https://watchman-batch-api.example.com/v1/batch/bulk-job \
...
**Check status:**
```bash
curl -s https://watchman-batch-api.example.com/v1/batch/bulk-job/{jobId} | jq '.'
```
...
# Submit via API
curl -X POST http://localhost:8084/v1/batch/bulk-job \
  -H 'Content-Type: application/json' \
  -d '{
    "jobName": "100k-test",
    "minMatch": 0.88,
...
- Verify Watchman API is running: `curl http://localhost:8084/v1/batch/bulk-job/health`
...
- Monitor job: `watch -n 10 'curl -s http://localhost:8084/v1/batch/bulk-job/{jobId} | jq'`
